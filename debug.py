...
def remove_inactive_ips():
    while True:
        ...
        with lock:
            ...
            # Print current state of dictionaries and sets
            print(f"ip_request_count: {ip_request_count}")
            print(f"suspected_ips: {suspected_ips}")
            print(f"confirm_ips: {confirm_ips}")
...
def before_request():
    ...
    with lock:
        ...
        if ip_address in suspected_ips:
            print(f"Request from suspected IP: {ip_address}")
        ...
        if ip_address in confirm_ips:
            print(f"Request from confirmed IP: {ip_address}")
        ...
        if total_request_count >= REQUEST_THRESHOLD:
            ...
            print("Server is busy. Adding IPs to suspected set.")
        ...
        if ip_request_count[ip_address] >= 5:
            ...
            print(f"IP moved to suspected set: {ip_address}")
    ...
    with lock:
        if ip_address in suspected_ips:
            if passcode == PASSCODE:
                ...
                ip_last_access_time[ip_address] = time.time()
                print(f"IP verified and removed from suspected set: {ip_address}")
                return redirect(url_for('home'))
            else:
                confirm_ips.add(ip_address)
                suspected_ips.remove(ip_address)
                print(f"IP failed verification and moved to confirm set: {ip_address}")
